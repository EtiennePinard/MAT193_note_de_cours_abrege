...
\end{note}
% Should I add this back in? Like it does not contribute that much to the rest of the document
% \begin{note}
...
Soient $A \in M_{m \times n}$ et $B \in M_{p \times q}$. Le produit matricielle $AB$ existe si et seulement si $n = p$. Dans ce cas, la matrice résultante est de taille $m \times q$ et l'élément à la position $k, j$ est calculé comme tel
...
Soient $A, B, C$ des matrices dont le produit matricielle entre eux existe. Alors on a les propriétés suivantes:
...
\begin{center}
    \begin{tabular}{c @{\hskip 1in} c}
        Associativité   & \underline{Non commutatif} (généralement) \\
        $(AB)C = A(BC)$ & $AB \neq BA$                              \\
        Élément neutre  & Distribution par la droite et gauche      \\
        $AI = IA = A$   & $A(B + C) = AB + AC$                      \\
                        & $(A + B)C = AC$
    \end{tabular}
\end{center}
